...
testApiQueue.process(async (job) => {
    const { metadata, responseHeaders, payload, timestamp } = job.data;

    const redisResult1 = await redis.lpush('eventlog', payload); 
    const redisResult2 = await redis.lpush('clickstream', JSON.stringify(metadata));
    const redisResult2 = await redis.lpush('clickstream.response_headers', JSON.stringify(responseHeaders));
...
app.post('/clicks', async (req, res) => {
    ...
    try {
        const job = await testApiQueue.add({
            ...
            responseHeaders: req.body.response.headers,
