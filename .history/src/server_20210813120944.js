...
function createLexicographicalKeySet(owner, slug, cfRay, timestamp) {
    ...
    const lexKey = createLexicographicalKey(timestamp, fragmentsArr);
    const userFragmentsArr = [...fragmentsArr, `${owner}`];
    const userLexKey = createLexicographicalKey(timestamp, userFragmentsArr);
    const slugFragmentsArr = [...fragmentsArr, `${slug}`, `${owner}`]; 
    const slugLexKey = createLexicographicalKey(timestamp, slugFragmentsArr); 
...
testApiQueue.process(async (job) => {
    ...
    let { lexKey, userLexKey, slugLexKey } = createLexicographicalKeySet(owner, slug, cfRay, timestamp); 
    ...
    try {
        ...
        const multiPromise = await redis.multi()
            ...
            .zadd('clickstream.chronological.by.user', 0, userLexicographicalKey)
            .zadd('clickstream.chronological.by.slug', 0, slugLexicographicalKey)
            .exec((err, result) => { 
