...
testApiQueue.process(async (job) => {
    const { cfRay, slug, owner, destination, logEntry, requestHeaders, responseHeaders, timestamp } = job.data;
    ...
    const redisResult4 = await redis.hset(`cfray.to.click`, cfRay, serializeParams(job.data)); 
