...
testApiQueue.process(async (job) => {
    ...
    console.log(`Adding job with timestamp: ${timestamp} to the queue at: ${timestamp2}`);
    ...
    return Promise.resolve(`The counter is now at: ${redisResult} at: ${timestamp3}`);
})
...
app.get('/route1', async (req, res) => {
    try {
        ...
        console.log(`Finished processing it with result: ${result}`);
